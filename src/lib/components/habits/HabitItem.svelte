...
<div class="flex items-center justify-between rounded-lg bg-white p-4 shadow-sm transition-all hover:shadow-md">
	<div class="flex items-center gap-4">
		<button
			onclick={handleToggle}
			class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border-2 transition-colors {
				isCompleted
					? 'border-green-500 bg-green-500 text-white'
					: 'border-gray-300 bg-transparent text-transparent hover:bg-gray-100'
			}"
			aria-label={isCompleted ? `Tandai "${habit.name}" sebagai belum selesai` : `Tandai "${habit.name}" sebagai selesai`}
		>
			<!-- Checkmark Icon -->
		...
		<span class="text-base font-medium text-gray-800 {isCompleted ? 'text-gray-400 line-through' : ''}">
			{habit.name}
		</span>
	</div>
